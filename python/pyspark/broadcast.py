    ...
    def __init__(self, sc=None, value=None, pickle_registry=None, path=None,
        ...
        if sc is not None:
            ...
            if sc._encryption_enabled:
                # with encryption, we ask the jvm to do the encryption for us, we sent it data
                # over a socket
                port, auth_secret = python_broadcast.setupEncryptionServer()
                sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                sock.connect(("127.0.0.1", port))
                encryption_sock_file = sock.makefile("rwb", 65536)
                do_server_auth(encryption_sock_file, auth_secret)
