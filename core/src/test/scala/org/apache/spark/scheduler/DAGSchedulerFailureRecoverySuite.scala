...
class DAGSchedulerFailureRecoverySuite extends SparkFunSuite with Logging {
  ...
  (0 until 100).foreach { idx =>
    test(s"no concurrent retries for stage attempts (SPARK-7308): trial $idx") {
    ...
    // could be enough to cause havoc.
      ...
      val conf = new SparkConf().set("spark.executor.memory", "400m")
      val clusterSc = new SparkContext("local-cluster[3,2,400]", "test-cluster", conf)
