...
class BroadcastTest(unittest.TestCase):
    ...
    def _test_encryption_helper(self, vs):
        ...
        value is the same when its read in the executors.  Also makes sure there are no task
        failures.
        """
...
class BroadcastFrameProtocolTest(unittest.TestCase):
    ...
    def _test_chunked_stream(self, data, py_buf_size):
        ...
        try:
            ...
            self._jvm.PythonBroadcast.readChunkedStream(jin, jout)
    ...
    def test_chunked_stream(self):
        def random_bytes(n):
            return bytearray(random.getrandbits(8) for _ in range(10))
